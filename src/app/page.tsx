...
export default function Home() {
  ...
  return <main>
    <section className="hero shell">
      ...
      <div className="hero-board" aria-label="Sprint preview">
        ...
        <div className="board-columns">
          ...
          <div><small>DONE</small><article>Research customer needs <strong>✓</strong></article></div>
        </div>
      </div>
    </section>

    <section className="section shell"><div className="section-intro"><div className="eyebrow">A smarter starting point</div><h2>Everything you need to move from brief to build.</h2></div><div className="feature-grid">
      {[["✦", "AI Roadmaps", "Bring a rough idea and leave with a structured product plan that your team can believe in."], ["▤", "Document intelligence", "Extract requirements, risks and delivery details from the documents you already have."], ["◔", "Performance analyzer", "Spot bottlenecks early with sprint health signals that make every stand-up more useful."]].map(([icon,title,text]) => <article className="feature" key={title}><span>{icon}</span><h3>{title}</h3><p>{text}</p><a href="#how">Learn more →</a></article>)}
    </div></section>
    ...
    <section className="section shell stats"><div><div className="eyebrow">Built for momentum</div><h2>Progress your whole team can see.</h2><p>Bring clarity to the work, so the important things keep moving.</p><Link className="text-link" href="/dashboard">Open your dashboard →</Link></div><div className="chart-card"><div><span>Sprint completion</span><b>78%</b></div><div className="bars"><i/><i/><i/><i/><i/><i/><i/></div><small>MON&nbsp;&nbsp;&nbsp; TUE&nbsp;&nbsp;&nbsp; WED&nbsp;&nbsp;&nbsp; THU&nbsp;&nbsp;&nbsp; FRI&nbsp;&nbsp;&nbsp; SAT&nbsp;&nbsp;&nbsp; SUN</small></div><div className="numbers"><b>12.4k <small>tasks completed</small></b><b>94% <small>on-time delivery</small></b><b>2.8× <small>faster planning</small></b></div></section>
    ...
    <section className="section shell"><div className="section-intro centered"><div className="eyebrow">From builders like you</div><h2>Clarity changes everything.</h2></div><div className="quotes">{[["DevSprint made our kickoff feel like day ten of a great project—not day one of a messy one.","Maya Chen","Product lead, Northstar"],["The AI plan gave our team the shared language we needed to actually start building.","Alex Morgan","Founder, Fieldwork"],["We now see risks before they derail a sprint. That alone has changed our pace.","Priya Shah","Engineering manager, Fable"]].map(([q,n,r])=><article key={n}><div>★★★★★</div><p>“{q}”</p><b>{n}</b><small>{r}</small></article>)}</div></section>
